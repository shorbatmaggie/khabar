...
# Weekly output directory mirrors Google workflow
WEEKLY_DIR = BASE_DIR / "data/digests/weekly"

# RSS digests live here (matches local_dedupe_rss.py)
...
def list_csv_files_with_dates(folder: Path) -> List[Tuple[date, Path]]:
    """
    Scan the folder for files named rss_articles_YYYY-MM-DD.csv
...
# === MAIN ===
def main() -> None:
    # Allow optional positional arg to override input directory (default matches local script)
    if len(sys.argv) >= 2:
    ...
    # Weekly output file: rss_weekly_<weekending>.csv
    end_str = week_ending.strftime("%Y-%m-%d")
    csv_out = WEEKLY_DIR / f"rss_weekly_{end_str}.csv"
